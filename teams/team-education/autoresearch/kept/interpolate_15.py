...
Method: linear in depth within each cast (running median down the cast for
turbidity and chlorophyll), then linear in time between the two nearest casts that
cover the depth, after removing a fitted 24-h cycle per 1-m depth bin and adding it
back at the query hour. Temperature is kept within the two bracketing casts.
"""
import numpy as np
import pandas as pd
...
CLIP = {"temp_c"}   # keep within the range of the two bracketing casts
...
def predict(train, queries):
    ...
    for col in PARAMS:
        ...
        res = np.empty(len(zq))
        lo = np.full(len(zq), -np.inf)
        hi = np.full(len(zq), np.inf)
        for i in range(len(zq)):
            ...
            if k == 0:
                ...
                if col in CLIP:   # the two bracketing casts as measured (cycle added back)
                    a, b = vv[k - 1:k + 1] + cyc(tt[k - 1:k + 1], zq[i])
                    lo[i], hi[i] = min(a, b), max(a, b)
        out[col] = np.clip(res + cyc(tq, zq), lo, hi)
